...
module.exports = (User, config, loopback) => {
  ...
  User.auth = (provider, req, res, callback) => {
    const authConfig = config.providers[provider],
          Provider = providerFactory.initProvider(provider, User, authConfig),
          currentContext = loopback.getCurrentContext(),
          id = currentContext && currentContext.active.accessToken &&
               currentContext.active.accessToken.userId || null;

    if (Provider.getInstance().getAuthType() === 'oauth1' &&
       (!req.body.oauth_token || !req.body.oauth_verifier)) {
      // perform authorization step of oauth1 requests
      authorize(Provider, req, res, callback);
    } else {
      if (id !== null) {
        // link user account with the given provider if request contains a valid accessToken
        link(Provider, id, req, callback);
      } else {
        // request authtoken if request does not contain a valid accessToken
        authenticate(Provider, req, callback);
      }
    }
  };
