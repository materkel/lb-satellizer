  ...
  return {
    ...
    findExistingUser: () => {
      return new Promise((resolve) => {
        let filter = {};
        const userId = instance.getUserData().userId;
        if (userId) {
          filter[name] = userId;
          User.findOne({ where: filter}, (err, user) => {
            if (user && user[name] === userId) {
              resolve(user);
            } else {
              reject(err);
            }
          ...
          reject(new Error('no valid provider userId'));
        }
      });
    },
    ...
    createProfile: () => {
      return new Promise((resolve, reject) => {
        const userData = instance.getUserData();
        if (userData.userEmail !== undefined && userData.userEmail !== null && userData.userEmail !== '') {
          User.findOne({ where: { email: userData.userEmail } }, (err, user) => {
            if (user && user.email === userData.userEmail) {
              user[name] = userData.userId;
              user.fullname = user.fullname || userData.userName;
              user.save(() => {
                resolve(user);
          ...
          reject(new Error('userData is undefined'));
        }
      });
    }
  };
